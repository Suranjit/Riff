...
- **`@riff/cli`** — `riff start`: generates session secrets and certificate,
  starts the host, serves the board, detects the LAN address, prints the join
  URL + code + fingerprint; `--demo` seeds sample capsules. (#12)
